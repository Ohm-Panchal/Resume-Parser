...
@app.route('/applicant/<email>/<name>', methods=['POST', 'GET'])
def applicant(email, name):
    custom_query_ans = ''
    if request.method == "POST":
        query = request.form['query']
        custom_query_ans = info_extract.get_answer(query)
    ...
    return render_template('applicant.html', edu = edu, wex = wex, proj = proj, ct = ct, jt = jt, data = data[0], custom_query_ans = custom_query_ans)
